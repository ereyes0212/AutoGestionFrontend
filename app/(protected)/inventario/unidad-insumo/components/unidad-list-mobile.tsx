...
interface UnidadInsumoListProps {
  unidades: UnidadInsumo[];
}

export default function UnidadInsumoListMobile({ unidades }: UnidadInsumoListProps) {
  ...
  return (
    <div className="space-y-4">
      <Link href={`/inventario/unidad-insumo/create`} className="w-full md:w-auto">
      ...
      {filteredUnidades.map((unidad) => (
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-500 truncate">{unidad.descripcion}</p>
            <Badge variant={unidad.activo ? "default" : "destructive"}>
              {unidad.activo ? "Activo" : "Inactivo"}
            </Badge>
          </div>
          <div className="flex items-center ml-4">
            <Link href={`/inventario/unidad-insumo/${unidad.id}/edit`}>
